...
def ponto_analise_alimentador_especifico(UTMX, UTMY,alimentadorespec):
    ###########################################
    ...
    def FiltroDist(df,utm_x, utm_y, passos: int, filtro_inicial: int, pontos: int, curvas = 1.4):
        ...
        if min(itens[:10]) > pontos and len(itens) > 10:
            df = deepcopy(aux)
    ...
    print('joao dicionario')
    print(dicionario)
    index = alimentadores.index(alimentadorespec)
    
    alimentador = alimentadores[index]
    equipamento = dicionario['Usina'][f'Ponto Ali {index+1}'][0]
    distancia = dicionario['Usina'][f'Distância Ali {index+1}'][0]
